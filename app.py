...
<style>
  .header-box {
    background:#0d3b6e;border-radius:12px;padding:14px 20px;
  ...
  .vpill          {background:rgba(255,255,255,0.15);border-radius:5px;
                   padding:2px 8px;font-size:10px;color:#e6f1fb;display:inline-block;margin-top:4px}
  .vdate          {color:#85b7eb;font-size:9px;margin-top:3px}
  .slabel         {font-size:11px;font-weight:bold;color:#0d3b6e;
  ...
  .result-box     {background:#eaf4fb;border-left:6px solid #0d3b6e;
                   border-radius:8px;padding:16px;margin-top:16px}
  .res-header     {display:flex;justify-content:space-between;margin-bottom:10px}
...
def load_and_train():
    def clean_num(series):
        return pd.to_numeric(
            series.replace({"***": np.nan, "NO": np.nan,
                             "SIN ESTIMULACIÓN": np.nan, "NINGUNO": np.nan}),
            errors="coerce")

    def clean_acceso(series):
        s = series.str.normalize("NFKD").str.encode("ascii", errors="ignore").str.decode("ascii").str.upper()
        return s.map(lambda x: 1 if "CATETER" in str(x) else (0 if "PUNCION" in str(x) else np.nan))

    df1 = pd.read_excel("BASE_2005_-_2025_ALO_IDL.xlsx", sheet_name="2013-2023")
    d1 = pd.DataFrame({
        "PRE_CD34"   : clean_num(df1["PRE CD34+/µL"]),
        "VOLEMIA"    : pd.to_numeric(df1["VOLEMIA mL"], errors="coerce"),
        "PESO_DON"   : pd.to_numeric(df1["PESO DON kg"], errors="coerce"),
        "PESO_REC"   : pd.to_numeric(df1["PESO REC. kg"], errors="coerce"),
        "EDAD_DON"   : pd.to_numeric(df1["EDAD DON "], errors="coerce"),
        "EDAD_REC"   : pd.to_numeric(df1["EDAD REC"], errors="coerce"),
        "PRE_WBC"    : pd.to_numeric(df1["PRE WBC  k/µL "], errors="coerce"),
        "PRE_MNC_PCT": pd.to_numeric(df1["PRE MNC%"], errors="coerce"),
        "PRE_MNC_ABS": pd.to_numeric(df1["PRE MNC k/µL"], errors="coerce"),
        "PRE_HTO"    : pd.to_numeric(df1["PRE HTO %"], errors="coerce"),
        "PRE_PLT"    : pd.to_numeric(df1["PRE PLT k/μL"], errors="coerce"),
        "DIAS_GCSF"  : clean_num(df1["DÍAS DE          G-CSF"]),
        "DOSIS_GCSF" : clean_num(df1["DOSIS DE          G-CSF POR DÍA"]),
        "PLERIXAFOR" : (clean_num(df1["DOSIS PLERIXAFOR mg/kg"]) > 0).astype(float),
        "TIPO_BIN"   : (df1["TIPO"] == "HAPLOIDÉNTICO").astype(int),
        "ACCESO_BIN" : clean_acceso(df1["ACCESO"]),
        "VEL_INICIAL": pd.to_numeric(df1["VEL. INICIAL mL/min"], errors="coerce"),
        "VEL_FINAL"  : pd.to_numeric(df1["VEL. FINAL mL/min"], errors="coerce"),
        "VOL_COSECHA": pd.to_numeric(df1["VOL. COSECHA mL"], errors="coerce"),
        "TARGET"     : pd.to_numeric(df1["VOLEMIAS PROCESADAS"], errors="coerce"),
    })

    df2 = pd.read_excel("2026_CPH.xlsx", sheet_name="Hoja1")
    d2 = pd.DataFrame({
        "PRE_CD34"   : clean_num(df2["CD34+ /µL DÍA 5"]),
        "VOLEMIA"    : pd.to_numeric(df2["VOLEMIA mL"], errors="coerce"),
        "PESO_DON"   : pd.to_numeric(df2["PESO DON"], errors="coerce"),
        "PESO_REC"   : pd.to_numeric(df2["PESO REC."], errors="coerce"),
        "EDAD_DON"   : pd.to_numeric(df2["EDAD DON "], errors="coerce"),
        "EDAD_REC"   : pd.to_numeric(df2["EDAD REC"], errors="coerce"),
        "PRE_WBC"    : pd.to_numeric(df2["PRE WBC  k/µL "], errors="coerce"),
        "PRE_MNC_PCT": pd.to_numeric(df2["PRE MNC%"], errors="coerce"),
        "PRE_MNC_ABS": pd.to_numeric(df2["PRE MNC k/µL"], errors="coerce"),
        "PRE_HTO"    : pd.to_numeric(df2["PRE HTO %"], errors="coerce"),
        "PRE_PLT"    : pd.to_numeric(df2["PRE PLT k/μL"], errors="coerce"),
        "DIAS_GCSF"  : clean_num(df2["DÍAS DE          G-CSF"]),
        "DOSIS_GCSF" : clean_num(df2["DOSIS DE          G-CSF"]),
        "PLERIXAFOR" : (clean_num(df2["DOSIS PLERIXAFOR mg/kg"]) > 0).astype(float),
        "TIPO_BIN"   : (df2["TIPO"] == "HAPLOIDÉNTICO").astype(int),
        "ACCESO_BIN" : clean_acceso(df2["ACCESO"]),
        "VEL_INICIAL": pd.to_numeric(df2["VEL. INICIAL mL/min"], errors="coerce"),
        "VEL_FINAL"  : pd.to_numeric(df2["VEL. FINAL mL/min"], errors="coerce"),
        "VOL_COSECHA": pd.to_numeric(df2["VOL. PRODUCTO"], errors="coerce"),
        "TARGET"     : pd.to_numeric(df2["VOLEMIAS PROCESADAS"], errors="coerce"),
    })
...
# ── Tipo de donador y acceso ──────────────────────────────────────────────────
st.markdown('<div class="slabel">Tipo de donador y acceso</div>', unsafe_allow_html=True)
col1, col2 = st.columns(2)
with col1:
    tipo = st.radio("Tipo de donador", ["Alogénico","Haploidéntico"],
                     horizontal=True, label_visibility="visible")
with col2:
    acceso = st.radio("Tipo de acceso", ["Catéter","Punción venosa"],
                       horizontal=True, label_visibility="visible")

vel_fin_imp = 85.0 if acceso == "Catéter" else 71.5
vel_tip_txt = f"Vel. final estimada: **{vel_fin_imp} mL/min** (mediana institucional · {acceso.lower()})"
st.markdown(f'<div class="itip">{vel_tip_txt}</div>', unsafe_allow_html=True)
...
with c1:
    ...
    cd34 = st.number_input("CD34+ /µL [10–100]", 10.0, 100.0, 50.0, 0.5,
...
with c3: plt     = st.number_input("PLT pre (k/µL)",    50.0, 600.0, 220.0, 5.0)
...
st.caption("Al activar, ingresa la dosis real administrada. "
           "Rango institucional: 0.12–0.27 mg/kg · Mediana: 0.24 mg/kg")

plerix_val = 0.0
if plerix_on:
    plerix_dosis = st.number_input("Dosis plerixafor (mg/kg)",
                                    0.10, 0.30, 0.24, 0.01)
    plerix_val   = 1.0
...
if calcular:
    ...
    acceso_bin = 1 if acceso == "Catéter" else 0

    X_new = pd.DataFrame([{
        "VOLEMIA"      : volemia,
        "VOL_COSECHA"  : vol_cos,
        "ACCESO_BIN"   : float(acceso_bin),
        "VEL_INICIAL"  : vel_ini,
        "PRE_HTO"      : hto,
    ...
    <div class="result-box">
      ...
      <div class="res-unit">volemias recomendadas</div>
      <div class="res-row">
      ...
      <div class="footer">
        ...
        Vel. final imputada: catéter=85 · punción=71.5 mL/min · CE=48%<br>
...
with st.expander("Historial de versiones"):
    versiones = [
        ("v4.5", "Abril 2026", "98", "0.327", "0.737",
         "Plerixafor con dosis ajustable y mg totales automáticos. App web Streamlit."),
        ("v4.4", "Abril 2026", "98", "0.327", "0.737",
         "Slider arriba, número abajo. Sin solapamiento visual."),
        ("v4.0", "Abril 2026", "98", "0.327", "0.737",
         "Vel. final imputada por mediana institucional según acceso (catéter=85, punción=71.5)."),
        ("v3.0", "Abril 2026", "98", "0.362", "0.690",
         "Se elimina vel. final del panel de entrada."),
        ("v2.0", "Abril 2026", "98", "0.327", "0.737",
         "Velocidades, vol. cosecha y filtro peso receptor ≥40 kg agregados."),
        ("v1.0", "Abril 2026", "279", "0.536", "0.642",
         "Modelo inicial ALO+HAPLO sin filtros de población."),
    ]
    for v, fecha, n, mae, r2, desc in versiones:
        st.markdown(
            f"**{v}** · {fecha} · n={n} · MAE={mae} · R²={r2}  
{desc}")
        st.divider()
